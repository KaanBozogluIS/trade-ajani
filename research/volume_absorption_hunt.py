"""Hacim-Fiyat Uyumsuzlugu (Absorbsiyon) Stratejisi icin tam-evren IS/OOS
taramasi. Ayni disiplin: her seri IS(%70)/OOS(%30) bolunur, parametre secimi
IS'e gore YAPILMAZ. Bir sonuc ancak IS VE OOS ikisinde de profit_factor>=1.0
ISE guvenilir sayilir (bkz. reversal_hunt.py'deki ayni disiplin/ONGUSDT dersi).

Kullanim:
    python research/volume_absorption_hunt.py
    python research/volume_absorption_hunt.py --timeframe 1h
"""
...
PARAM_GRID: list[dict] = [
    ...
    {"absorb_vol_mult": 1.5, "absorb_range_mult": 0.9, "tp_r_mult": 2.0, "sl_atr_buffer": 0.3,
     "tolerance_pct": 0.15, "min_touches": 2, "require_trend_align": False},
    {"absorb_vol_mult": 1.2, "absorb_range_mult": 1.1, "tp_r_mult": 2.5, "sl_atr_buffer": 0.3,
     "tolerance_pct": 0.15, "min_touches": 2, "require_trend_align": True},
    {"absorb_vol_mult": 1.1, "absorb_range_mult": 1.3, "tp_r_mult": 2.5, "sl_atr_buffer": 0.4,
     "tolerance_pct": 0.2, "min_touches": 2, "require_trend_align": False},
    {"absorb_vol_mult": 1.2, "absorb_range_mult": 1.1, "tp_r_mult": 1.8, "sl_atr_buffer": 0.3,
     "tolerance_pct": 0.15, "min_touches": 3, "require_trend_align": False},
...
def evaluate(df: pd.DataFrame, params: dict, timeframe: str, market: str, costs: Costs) -> dict | None:
    if len(df) < 250:
        return None
    result = run_backtest(df, VolumeAbsorption(**params), costs=costs)
...
def main() -> None:
    ...
    out_path = Path(__file__).resolve().parent.parent / "reports" / "volume_absorption_hunt_sonuclari.csv"
