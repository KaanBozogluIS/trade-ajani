"""Log-Regresyon Koridoru Stratejisi icin tam-evren IS/OOS taramasi.
Ayni disiplin: IS(%70)/OOS(%30), profit_factor>=1.0 HEM IS HEM OOS'ta.

Kullanicinin TradingView'de kullandigi "Log-Regression Probability
Corridor" indikatorunun NEDENSEL (lookahead'siz) versiyonu - bkz.
core/strategies/log_regresyon_koridoru.py docstring'i.

Kullanim:
    python research/log_regresyon_hunt.py
    python research/log_regresyon_hunt.py --timeframe 1h
"""
...
from core.strategies.log_regresyon_koridoru import LogRegressionCorridor

CONFIG_PATH = Path(__file__).resolve().parent.parent / "config" / "universe.yaml"

PARAM_GRID: list[dict] = [
    {"window": 500, "entry_z": 2.0, "stop_z": 3.0, "target_z": 0.0},
    {"window": 1000, "entry_z": 2.0, "stop_z": 3.0, "target_z": 0.0},
    {"window": 500, "entry_z": 2.5, "stop_z": 3.5, "target_z": 0.0},
    {"window": 1000, "entry_z": 2.5, "stop_z": 3.5, "target_z": 0.5},
    {"window": 1500, "entry_z": 2.0, "stop_z": 3.0, "target_z": 0.0},
    {"window": 750, "entry_z": 2.2, "stop_z": 3.2, "target_z": 0.0},
]
...
def evaluate(df: pd.DataFrame, params: dict, timeframe: str, market: str, costs: Costs) -> dict | None:
    if len(df) < 250:
        return None
    result = run_backtest(df, LogRegressionCorridor(**params), costs=costs)
...
def main() -> None:
    ...
    MIN_TOTAL_BARS = 2000  # bu strateji window>=500 gerektiriyor, daha yuksek esik
    MIN_TRADES = 5
    ...
    for job_i, (provider, symbol, tf, market) in enumerate(jobs):
        ...
        for params in PARAM_GRID:
            ...
            rows.append({
                ...
                "OOS_kazanma_%": m_oos["kazanma_orani_%"], "IS_maxdd_%": m_is["max_drawdown_%"],
                "OOS_maxdd_%": m_oos["max_drawdown_%"],
    ...
    out_path = Path(__file__).resolve().parent.parent / "reports" / "log_regresyon_hunt_sonuclari.csv"
    ...
    display_cols = ["provider", "symbol", "tf", "IS_sharpe", "OOS_sharpe", "IS_profit_factor",
                     "OOS_profit_factor", "OOS_islem", "OOS_kazanma_%", "IS_maxdd_%", "OOS_maxdd_%", "params"]
    if not passed.empty:
